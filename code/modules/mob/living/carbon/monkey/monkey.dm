...
/mob/living/carbon/monkey/update_clothing()
	...
	if (src.wear_mask)
		if (istype(src.wear_mask, /obj/item/clothing/mask))
			...
			src.overlays += image("icon" = 'icons/mob/monkey.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = src.layer)
		src.wear_mask.screen_loc = ui_mask
	if (src.r_hand)
		src.overlays += image("icon" = 'icons/mob/items_righthand.dmi', "icon_state" = src.r_hand.item_state ? src.r_hand.item_state : src.r_hand.icon_state, "layer" = src.layer)
		src.r_hand.screen_loc = ui_rhand

	if (src.l_hand)
		src.overlays += image("icon" = 'icons/mob/items_lefthand.dmi', "icon_state" = src.l_hand.item_state ? src.l_hand.item_state : src.l_hand.icon_state, "layer" = src.layer)
		src.l_hand.screen_loc = ui_lhand

	if (src.back)
		if (!( src.lying ))
			src.overlays += image("icon" = 'icons/mob/monkey.dmi', "icon_state" = "back", "layer" = src.layer)
		else
			src.overlays += image("icon" = 'icons/mob/monkey.dmi', "icon_state" = "back2", "layer" = src.layer)
		src.back.screen_loc = ui_back
	if (src.handcuffed)
		src.pulling = null
		if (!( src.lying ))
			src.overlays += image("icon" = 'icons/mob/monkey.dmi', "icon_state" = "handcuff1", "layer" = src.layer)
		else
			src.overlays += image("icon" = 'icons/mob/monkey.dmi', "icon_state" = "handcuff2", "layer" = src.layer)
	if (src.client)

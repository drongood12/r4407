/mob/living/carbon/human/New()
	...
	spawn (1)
		...
		if(!src.stand_icon)
			src.stand_icon = new /icon('icons/mob/human.dmi', "body_[g]_s")
		if(!src.lying_icon)
			src.lying_icon = new /icon('icons/mob/human.dmi', "body_[g]_l")
...
/mob/living/carbon/human/update_clothing()
	...
	if (src.mutations & 8)
		src.overlays += image("icon" = 'icons/effects/genetics.dmi', "icon_state" = "hulk[fat][!src.lying ? "_s" : "_l"]")

	if (src.mutations & 2)
		src.overlays += image("icon" = 'icons/effects/genetics.dmi', "icon_state" = "fire[fat][!src.lying ? "_s" : "_l"]")

	if (src.mutations & 1)
		src.overlays += image("icon" = 'icons/effects/genetics.dmi', "icon_state" = "telekinesishead[fat][!src.lying ? "_s" : "_l"]")

	if (src.mutantrace)
		src.overlays += image("icon" = 'icons/effects/genetics.dmi', "icon_state" = "[mutantrace][fat][!src.lying ? "_s" : "_l"]")
	...
	//	src.zone_sel.overlays += image("icon" = 'icons/mob/zone_sel.dmi', "icon_state" = text("[]", src.zone_sel.selecting))
	...
	if (src.w_uniform)
		...
		if (istype(src.w_uniform, /obj/item/clothing/under))
			...
			if (!t1)
				t1 = src.icon_state
			if (src.mutations & 32)
				src.overlays += image("icon" = 'icons/mob/uniform_fat.dmi', "icon_state" = "[t1][!src.lying ? "_s" : "_l"]", "layer" = MOB_LAYER)
			else
				src.overlays += image("icon" = 'icons/mob/uniform.dmi', "icon_state" = text("[][]",t1, (!(src.lying) ? "_s" : "_l")), "layer" = MOB_LAYER)
			if (src.w_uniform.blood_DNA)
				var/icon/stain_icon = icon('icons/effects/blood.dmi', "uniformblood[!src.lying ? "" : "2"]")
				src.overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)

	if (src.wear_id)
		src.overlays += image("icon" = 'icons/mob/mob.dmi', "icon_state" = "id[!src.lying ? null : "2"]", "layer" = MOB_LAYER)
	...
	if (src.gloves)
		...
		src.overlays += image("icon" = 'icons/mob/hands.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER)
		if (src.gloves.blood_DNA)
			var/icon/stain_icon = icon('icons/effects/blood.dmi', "bloodyhands[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	...
	if (src.glasses)
		...
		src.overlays += image("icon" = 'icons/mob/eyes.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER)
	// Ears
	if (src.ears)
		var/t1 = src.ears.icon_state
		src.overlays += image("icon" = 'icons/mob/ears.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER)
	// Shoes
	if (src.shoes)
		var/t1 = src.shoes.icon_state
		src.overlays += image("icon" = 'icons/mob/feet.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER)
		if (src.shoes.blood_DNA)
			var/icon/stain_icon = icon('icons/effects/blood.dmi', "shoesblood[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)	// Radio
/*	if (src.w_radio)
		src.overlays += image("icon" = 'icons/mob/ears.dmi', "icon_state" = "headset[!src.lying ? "" : "2"]", "layer" = MOB_LAYER) */
	...
	if (src.wear_mask)
		if (istype(src.wear_mask, /obj/item/clothing/mask))
			...
			src.overlays += image("icon" = 'icons/mob/mask.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER)
			if (!istype(src.wear_mask, /obj/item/clothing/mask/cigarette))
				if (src.wear_mask.blood_DNA)
					var/icon/stain_icon = icon('icons/effects/blood.dmi', "maskblood[!src.lying ? "" : "2"]")
					src.overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	...
	if (src.wear_suit)
		...
		if (istype(src.wear_suit, /obj/item/clothing/suit))
			...
			src.overlays += image("icon" = 'icons/mob/suit.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER)
		if (src.wear_suit.blood_DNA)
			...
			if (istype(src.wear_suit, /obj/item/clothing/suit/armor/vest || /obj/item/clothing/suit/wcoat || /obj/item/clothing/suit/armor/a_i_a_ptank))
				stain_icon = icon('icons/effects/blood.dmi', "armorblood[!src.lying ? "" : "2"]")
			else if (istype(src.wear_suit, /obj/item/clothing/suit/det_suit || /obj/item/clothing/suit/labcoat))
				stain_icon = icon('icons/effects/blood.dmi', "coatblood[!src.lying ? "" : "2"]")
			else
				stain_icon = icon('icons/effects/blood.dmi', "suitblood[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	...
	if (src.head)
		...
		var/icon/head_icon = icon('icons/mob/head.dmi', text("[][]", t1, (!( src.lying ) ? null : "2")))
		src.overlays += image("icon" = head_icon, "layer" = MOB_LAYER)
		if (src.head.blood_DNA)
			var/icon/stain_icon = icon('icons/effects/blood.dmi', "helmetblood[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	...
	if (src.belt)
		...
		src.overlays += image("icon" = 'icons/mob/belt.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER)
	...
	if (src.back)
		...
		src.overlays += image("icon" = 'icons/mob/back.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER)
		src.back.screen_loc = ui_back

	if (src.handcuffed)
		src.pulling = null
		if (!src.lying)
			src.overlays += image("icon" = 'icons/mob/mob.dmi', "icon_state" = "handcuff1", "layer" = MOB_LAYER)
		else
			src.overlays += image("icon" = 'icons/mob/mob.dmi', "icon_state" = "handcuff2", "layer" = MOB_LAYER)
	...
	if (src.r_hand)
		src.overlays += image("icon" = 'icons/mob/items_righthand.dmi', "icon_state" = src.r_hand.item_state ? src.r_hand.item_state : src.r_hand.icon_state, "layer" = MOB_LAYER+1)

		src.r_hand.screen_loc = ui_rhand

	if (src.l_hand)
		src.overlays += image("icon" = 'icons/mob/items_lefthand.dmi', "icon_state" = src.l_hand.item_state ? src.l_hand.item_state : src.l_hand.icon_state, "layer" = MOB_LAYER+1)
	...
	if (shielded)
		src.overlays += image("icon" = 'icons/mob/mob.dmi', "icon_state" = "shield", "layer" = MOB_LAYER)
...
/mob/living/carbon/human/proc/update_body()
	...
	src.stand_icon = new /icon('icons/mob/human.dmi', "blank")
	src.lying_icon = new /icon('icons/mob/human.dmi', "blank")

	var/husk = (src.mutations & 64)
	var/obese = (src.mutations & 32)

	if (husk)
		src.stand_icon.Blend(new /icon('icons/mob/human.dmi', "husk_s"), ICON_OVERLAY)
		src.lying_icon.Blend(new /icon('icons/mob/human.dmi', "husk_l"), ICON_OVERLAY)
	else if(obese)
		src.stand_icon.Blend(new /icon('icons/mob/human.dmi', "fatbody_s"), ICON_OVERLAY)
		src.lying_icon.Blend(new /icon('icons/mob/human.dmi', "fatbody_l"), ICON_OVERLAY)
	else
		src.stand_icon.Blend(new /icon('icons/mob/human.dmi', "chest_[g]_s"), ICON_OVERLAY)
		src.lying_icon.Blend(new /icon('icons/mob/human.dmi', "chest_[g]_l"), ICON_OVERLAY)

		for (var/part in list("head", "arm_left", "arm_right", "hand_left", "hand_right", "leg_left", "leg_right", "foot_left", "foot_right"))
			src.stand_icon.Blend(new /icon('icons/mob/human.dmi', "[part]_s"), ICON_OVERLAY)
			src.lying_icon.Blend(new /icon('icons/mob/human.dmi', "[part]_l"), ICON_OVERLAY)

		src.stand_icon.Blend(new /icon('icons/mob/human.dmi', "groin_[g]_s"), ICON_OVERLAY)
		src.lying_icon.Blend(new /icon('icons/mob/human.dmi', "groin_[g]_l"), ICON_OVERLAY)
	...
	if (src.underwear > 0)
		if(!obese)
			src.stand_icon.Blend(new /icon('icons/mob/human.dmi', "underwear[src.underwear]_[g]_s"), ICON_OVERLAY)
			src.lying_icon.Blend(new /icon('icons/mob/human.dmi', "underwear[src.underwear]_[g]_l"), ICON_OVERLAY)
...
/mob/living/carbon/human/proc/update_face()
	...
	var/icon/hair_l = new/icon("icon" = 'icons/mob/human_face.dmi', "icon_state" = "[src.hair_icon_state]_l")
	...
	var/icon/facial_l = new/icon("icon" = 'icons/mob/human_face.dmi', "icon_state" = "[src.face_icon_state]_l")
